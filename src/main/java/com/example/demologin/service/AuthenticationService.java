...
public class AuthenticationService implements UserDetailsService {
    ...
    public User register(UserRegistrationRequest request) {
        ...
        try {
            ...
            // Log activity for failed registration
            log = UserActivityLog.builder()
                    .activityType(ActivityType.REGISTRATION)
                    .timestamp(LocalDateTime.now())
                    .status("FAIL")
    ...
    public UserResponse authenticateWithGoogle(GoogleLoginRequest request) {
        try {
            // Validate the token is not null or empty
            ...
            // Check if the token starts with "ya29." which indicates it's an access token, not an ID token
            ...
            // If it's not an access token, try to verify it as an ID token
            ...
            try {
                ...
                // Handle the specific null argument exception
    ...
    private UserResponse authenticateWithGoogleAccessToken(String accessToken) {
        try {
            // Use RestTemplate to call Google's userinfo API with the access token
            ...
            // Now that we have the email and name, we can authenticate the user
    ...
    public UserResponse authenticateWithOAuth2(String email, String name) {
        try {
            ...
            if (user == null) {
                // Create new user with information from OAuth2
                user = User.builder()
                        .username(email.substring(0, email.indexOf('@')))
                        .fullName(name)
                        .email(email)
    ...
    public UserResponse authenticateWithFacebook(FacebookLoginRequest request) {
        try {
            ...
            // Request more fields from Facebook
            ...
            if (email == null) {
                // Some Facebook users might not have email, so we'll generate one
                String userId = (String) userInfo.get("id");
                email = userId + "@facebook.com";
            }

            // Now create or update user with all the data we got
        ...
        try {
            ...
            if (user == null) {
                ...
                user = User.builder()
                        ...
                        .email(email)
                        .password(passwordEncoder.encode("")) // Mật khẩu rỗng cho login social
                        .role(Role.MEMBER) // Luôn set role là MEMBER
                        .status(UserStatus.ACTIVE) // Luôn set status là ACTIVE
                        .createdAt(LocalDateTime.now())
                        .phone("")
                ...
                // Đảm bảo role và status luôn đúng
    ...
    private LocalDate parseBirthday(String birthday) {
        if (birthday == null || birthday.isEmpty()) {
            return LocalDate.now().minusYears(18); // Default to 18 years ago
        }

        try {
            // Facebook birthday format can be MM/DD/YYYY or YYYY-MM-DD
        ...
        return LocalDate.now().minusYears(18); // Fallback to 18 years ago
    }
